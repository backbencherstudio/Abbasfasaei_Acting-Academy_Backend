...
export class CommunityService {
  ...
  async createPost(
    ...
    const post = await this.prisma.communityPost.create({
      data: {
        ...
        ...(friends_ids?.length > 0 && {
          allowed_friends: { connect: friends_ids.map((id) => ({ id })) },
        }),
      },
      select: {
        id: true,
  ...
  async updatePost(
    ...
    if (post_type && post_type !== existingPost.post_type) {
    ...
    const post = await this.prisma.communityPost.update({
      ...
      data: {
        ...
        ...(friends_ids?.length > 0 && {
          allowed_friends: { set: friends_ids.map((id) => ({ id })) },
        }),
      },
      select: {
        id: true,
  ...
  async getLikes(
    post_id: string,
    user_id: string,
    query: QueryCommunityPostLikesDto,
  ) {
    if (!user_id) throw new UnauthorizedException('user not found');
    if (!post_id) throw new BadRequestException('Invalid post id');
    ...
    return {
      ...
      data: {
        ...
        cover_image: user.cover_image
          ? NajimStorage.url(user.cover_image)
          : null,
      },
    };
  }
